...
namespace appBugInn
{
    public partial class F_checkOut : MaterialForm
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(txt_idQuartoCheckOut.Text))
            {
                MessageBox.Show("Nenhum check-in selecionado.");
                return;
            }

            int id = int.Parse(txt_idQuartoCheckOut.Text);
            ...
            if (pagamento == "Outro") pagamento = txt_outroPagamento.Text.Trim();
            ...
            float classificacao = float.Parse(cb_review.SelectedItem.ToString().Split(' ')[0]); // pega só o número


            //hotel.preencherFaturamentos();
